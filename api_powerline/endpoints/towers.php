...
function registerTowerRoutes(Router $router): void
{
    ...
    $router->post('towers', function () {
        ...
        if ($lineId !== null) {
            $line = $db->fetchOne("SELECT id, line_code, line_supervisor FROM `lines` WHERE id = ?", [$lineId]);
            if (!$line) {
        ...
        // v2.1.0: نوع دکل (enum) از ساختار دکل مشتق می‌شود و از سمت کلاینت ارسال نمی‌شود
        $typeByStructure = [
            'مشبک فلزی' => 'lattice_steel',
            'تیر چوبی' => 'wood',
            'تیر بتنی' => 'concrete',
            'تلسکوپی بتنی' => 'concrete_tele',
            'تلسکوپی فلزی' => 'steel_tele',
        ];
        $towerType = $typeByStructure[$body['tower_structure']] ?? 'other';
        ...
        // v2.1.0: ساختار بر اساس اکسل رسمی — نوع مقره هر فاز/مدار جداگانه
        $sql = "INSERT INTO towers
                (line_id, tower_code, tower_number, tower_type, tower_structure,
                 foundation_type_code, foundation_type,
                 ...
                 gps_lat, gps_lng, geom, line_supervisor, is_active, created_at)
                VALUES
        ...
        $db->execute($sql, [
            ...
            $body['tower_number'],
            $towerType,
            $body['tower_structure'],
            $body['foundation_type_code'] ?? null,
            $body['foundation_type'],
            $body['base_height_a'] ?? null,
            $body['base_height_b'] ?? null,
            $body['base_height_c'] ?? null,
            $body['base_height_d'] ?? null,
            $body['insulator_r1'] ?? null,
            $body['insulator_s1'] ?? null,
            $body['insulator_t1'] ?? null,
            $body['insulator_r2'] ?? null,
            $body['insulator_s2'] ?? null,
            $body['insulator_t2'] ?? null,
            $body['insulator_count_r1'] ?? null,
            $body['insulator_count_s1'] ?? null,
            $body['insulator_count_t1'] ?? null,
            $body['insulator_count_r2'] ?? null,
            $body['insulator_count_s2'] ?? null,
            $body['insulator_count_t2'] ?? null,
            $gpsLat,
            $gpsLng,
            $geomWkt,
            $supervisor,
    ...
    $router->put('towers/{id}', function ($id) {
        ...
        $allowedFields = [
            'tower_number', 'tower_structure', 'foundation_type_code', 'foundation_type',
            'base_height_a', 'base_height_b', 'base_height_c', 'base_height_d',
            'insulator_r1', 'insulator_s1', 'insulator_t1', 'insulator_r2', 'insulator_s2', 'insulator_t2',
            'insulator_count_r1', 'insulator_count_s1', 'insulator_count_t1',
            'insulator_count_r2', 'insulator_count_s2', 'insulator_count_t2',
        ...
        if ((array_key_exists('line_id', $body) || array_key_exists('tower_number', $body)) && $effectiveLineId && $effectiveNumber) {
            ...
            if ($lineRow) {
                ...
                // نوع دکل (enum) هم اگر ساختار عوض شده مشتق می‌شود
                if (array_key_exists('tower_structure', $body)) {
                    $typeByStructure = [
                        'مشبک فلزی' => 'lattice_steel', 'تیر چوبی' => 'wood', 'تیر بتنی' => 'concrete',
                        'تلسکوپی بتنی' => 'concrete_tele', 'تلسکوپی فلزی' => 'steel_tele',
                    ];
                    $updates[] = "tower_type = ?";
                    $params[] = $typeByStructure[$body['tower_structure']] ?? 'other';
                }
            }
        }
    ...
    $router->delete('towers/{id}', function ($id) {
        ...
        $existing = $db->fetchOne("SELECT id FROM towers WHERE id = ?", [$towerId]);
        if (!$existing) {
            Response::error(404, 'دکل پیدا نشد');
        }
    ...
    $router->post('towers/bulk-import', function () {
        ...
        try {
            ...
            $ins = $pdo->prepare(
                "INSERT INTO towers (line_id, tower_code, tower_number, tower_type, tower_structure,
                 foundation_type_code, foundation_type, base_height_a, base_height_b, base_height_c, base_height_d,
                 insulator_r1, insulator_s1, insulator_t1, insulator_r2, insulator_s2, insulator_t2,
                 insulator_count_r1, insulator_count_s1, insulator_count_t1,
                 insulator_count_r2, insulator_count_s2, insulator_count_t2,
                 gps_lat, gps_lng, geom, line_supervisor, is_active, created_at)
                 VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,ST_GeomFromText(?),?,1,NOW())"
            );
            $upd = $pdo->prepare(
                "UPDATE towers SET line_id=?, tower_number=?, tower_structure=?, foundation_type_code=?, foundation_type=?,
                 base_height_a=?, base_height_b=?, base_height_c=?, base_height_d=?,
                 insulator_r1=?, insulator_s1=?, insulator_t1=?, insulator_r2=?, insulator_s2=?, insulator_t2=?,
                 insulator_count_r1=?, insulator_count_s1=?, insulator_count_t1=?,
                 insulator_count_r2=?, insulator_count_s2=?, insulator_count_t2=?,
                 gps_lat=?, gps_lng=?, geom=ST_GeomFromText(?), line_supervisor=?
                 WHERE id=?"
            );
            ...
            foreach ($rows as $i => $r) {
                try {
                    ...
                    if (!empty($r['id'])) {
                        ...
                        $upd->execute([
                            $lineId, $number, $structure, $n('foundation_type_code'), $n('foundation_type'),
                            $n('base_height_a'), $n('base_height_b'), $n('base_height_c'), $n('base_height_d'),
                            $n('insulator_r1'), $n('insulator_s1'), $n('insulator_t1'), $n('insulator_r2'), $n('insulator_s2'), $n('insulator_t2'),
                            $n('insulator_count_r1'), $n('insulator_count_s1'), $n('insulator_count_t1'),
                            $n('insulator_count_r2'), $n('insulator_count_s2'), $n('insulator_count_t2'),
                            $gpsLat, $gpsLng, $geomWkt, $supervisor, (int) $r['id'],
                        ]);
                        ...
                        $ins->execute([
                            $lineId, $code, $number, $towerType, $structure,
                            $n('foundation_type_code'), $n('foundation_type'),
                            $n('base_height_a'), $n('base_height_b'), $n('base_height_c'), $n('base_height_d'),
                            $n('insulator_r1'), $n('insulator_s1'), $n('insulator_t1'), $n('insulator_r2'), $n('insulator_s2'), $n('insulator_t2'),
                            $n('insulator_count_r1'), $n('insulator_count_s1'), $n('insulator_count_t1'),
                            $n('insulator_count_r2'), $n('insulator_count_s2'), $n('insulator_count_t2'),
                            $gpsLat, $gpsLng, $geomWkt, $supervisor,
                        ]);
...
function formatTowerRow(array $row): array
{
    ...
    return [
        ...
        'foundation_type_code' => $row['foundation_type_code'] ?? null,
