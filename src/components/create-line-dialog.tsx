...
interface FormData {
  ...
  conductor_type: string; tower_structure: string; length_km: string; circuit_length_km: string;
...
export function CreateLineDialog({ open, onClose, onCreated, editRow, duplicateFrom }: Props) {
  ...
  const { structures: towerStructures } = useTowerReferences(open);
    ...
    if (open) {
      ...
      if (sourceRow) {
        setForm({
          ...
          tower_structure: sourceRow.tower_structure || "",
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        tower_structure: form.tower_structure || null,
          ...
          <div className="space-y-3 p-4 bg-white dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <Field label="ساختار دکل">
                {(() => {
                  const locked = Boolean(isEdit && Number(editRow?.tower_count ?? 0) > 0);
                  const options = towerStructures.map(s => ({ value: s.name, label: s.name }));
                  const current = form.tower_structure;
                  const safeOptions = current && !options.some(o => o.value === current) ? [...options, { value: current, label: current }] : options;
                  return <>
                    <SearchableSelect
                      value={current}
                      onChange={v => set("tower_structure", v)}
                      options={safeOptions}
                      placeholder="انتخاب ساختار دکل..."
                      searchPlaceholder="جستجوی ساختار..."
                      allowClear={!locked}
                      disabled={locked}
                    />
                    {locked && <p className="mt-1 flex items-center gap-1 text-[11px] text-amber-600 dark:text-amber-400"><LockKeyhole className="w-3 h-3" />بعد از ثبت دکل‌ها، ساختار خط از پرتکرارترین ساختار دکل‌ها محاسبه می‌شود و این فیلد قابل تغییر نیست.</p>}
                  </>;
                })()}
              </Field>
            </div>
