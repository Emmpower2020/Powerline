...
type FieldAction =
  | "tower_structure" | "foundation_type_code" | "foundation_type"
...
const actionMeta: Record<FieldAction, { title: string; label: string; placeholder?: string }> = {
  ...
  foundation_type_code: { title: "تغییر گروهی نوع دکل", label: "نوع دکل (NN، LT و...)", placeholder: "مثلاً: NN" },
...
export function BulkTowersActions({ getSelection, onApplied }: BulkTowersActionsProps) {
  ...
  const confirmField = async () => {
    ...
    const isText = ["tower_structure", "foundation_type_code", "line_supervisor"].includes(fieldAction);
    ...
    switch (fieldAction) {
      ...
      case "foundation_type_code": patch = { foundation_type_code: value.trim() }; break;
    ...
    <>
      <DropdownMenu dir="rtl">
        ...
        <DropdownMenuContent align="start" className="w-60">
          ...
          <ItemRow icon={<Radio className="w-4 h-4 text-indigo-600" />} label="نوع دکل" onClick={() => startFieldAction("foundation_type_code")} />
