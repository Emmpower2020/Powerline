...
import { TOWER_STRUCTURE_OPTIONS } from "@/components/create-line-dialog";
...
type FieldAction =
  ...
  | "voltage" | "conductor_type" | "tower_structure_type";
...
const actionMeta: Record<FieldAction, { title: string; label: string; placeholder?: string }> = {
  ...
  tower_structure_type:{ title: "تغییر گروهی نوع سازه دکل", label: "نوع سازه دکل", placeholder: "مثلاً: مشبک فلزی" },
};
...
export function BulkLinesActions({ getSelection, onApplied }: BulkLinesActionsProps) {
  ...
  const confirmField = async () => {
    ...
    switch (fieldAction) {
      ...
      case "tower_structure_type": patch = { tower_structure_type: value.trim() }; break;
    ...
    <>
      <DropdownMenu dir="rtl">
        ...
        <DropdownMenuContent align="start" className="w-60">
          ...
          <ItemRow icon={<Building2 className="w-4 h-4 text-slate-600" />} label="نوع سازه دکل" onClick={() => startFieldAction("tower_structure_type")} />
          <DropdownMenuSeparator />
      ...
      <Dialog open={dialogOpen} onOpenChange={(o) => { if (!o && !applying) setFieldAction(null); }}>
        <DialogContent className="max-w-md" dir="rtl">
          ...
          <div className="space-y-3">
            ...
            ) : fieldAction === "tower_structure_type" ? (
              // v3.4.1: نوع سازه دکل از مقادیر معتبر دیتابیس
              <div className="space-y-2">
                <Label className="text-right block">{actionMeta.tower_structure_type.label}</Label>
                <SearchableSelect
                  value={value}
                  onChange={setValue}
                  options={TOWER_STRUCTURE_OPTIONS}
