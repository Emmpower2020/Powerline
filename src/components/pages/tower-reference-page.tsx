...
export function TowerReferencePage({ kind }: { kind: "structures" | "type-codes" }) {
  ...
  return <div className="space-y-3">
    ...
    <Dialog open={open} onOpenChange={setOpen}><DialogContent><DialogHeader><DialogTitle className="text-right">{edit?`ویرایش ${singular}`:`ثبت ${singular} جدید`}</DialogTitle></DialogHeader><div className="space-y-4 text-right">
      <div className="space-y-2"><Label className="text-right block text-xs text-slate-600 dark:text-slate-300">{kind==="structures"?"نام ساختار":"کد"}</Label><Input value={value} onChange={e=>setValue(e.target.value)} dir="rtl" /></div>
      {kind==="type-codes"&&<div className="space-y-2"><Label className="text-right block text-xs text-slate-600 dark:text-slate-300">عنوان</Label><Input value={title} onChange={e=>setTitle(e.target.value)} /></div>}
      <div className="space-y-2"><Label className="text-right block text-xs text-slate-600 dark:text-slate-300">ترتیب نمایش</Label><Input type="number" value={sort} onChange={e=>setSort(e.target.value)} dir="ltr" /></div>
      <div className="flex items-center justify-between border rounded-lg px-3 py-2"><Label className="text-xs text-slate-600 dark:text-slate-300">فعال</Label><Switch checked={active} onCheckedChange={setActive}/></div>
