...
import {
  ...
  Plus, Download, Eye, EyeOff, Filter, X, Check, Copy, Trash2, ArrowUp, ArrowDown, Pencil,
} from "lucide-react";
import {
...
function DataTableInner<T extends { id: number }>({
  ...
  onAdd, onRefresh, onRowClick, onCopy, onDelete, onEdit,
  pageSize = 20, searchable = true, tableRef,
}: DataTableProps<T>) {
  const [search, setSearch] = useState("");
  const [sortKey, setSortKey] = useState<string | null>(null);
  const [sortDir, setSortDir] = useState<SortState>("none");
  const [page, setPage] = useState(1);
  const [hiddenColumns, setHiddenColumns] = useState<Set<string>>(new Set(columns.filter(c => c.hidden).map(c => c.key)));
  ...
  const toggleSelectAll = () => { if (selectedRows.size === paginated.length) setSelectedRows(new Set()); else setSelectedRows(new Set(paginated.map(r => r.id))); };
  ...
  // Export visible filtered rows as CSV (with header)
  const handleExport = () => {
    ...
    const rows = filtered.map(row => visibleColumns.map(c => escape(row[c.key as keyof T])).join(","));
    ...
    <div className="space-y-3" dir="rtl">
      ...
      <div className="flex flex-wrap items-center gap-2 justify-between">
        <div className="flex items-center gap-1 flex-wrap">
          ...
          {onCopy && (
            <Button variant="outline" size="icon" onClick={handleCopy} title="کپی (TSV برای اکسل)" className="h-9 w-9">
          ...
          {/* Inline selection counter — placed between action buttons and column menu */}
          {selCount > 0 && (
          ...
          {/* Column Settings - custom popover for proper RTL */}
          <div className="relative" ref={columnMenuRef}>
            <Button variant="outline" size="icon" title="ستون‌ها" className="h-9 w-9" onClick={() => setShowColumnMenu(o => !o)}>
              <EyeOff className="w-4 h-4" />
            </Button>
            {showColumnMenu && (
              <div className="absolute z-50 mt-1 right-0 w-72 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg" dir="rtl">
          ...
          <Button variant="outline" size="icon" onClick={handleExport} title="خروجی CSV" className="h-9 w-9"><Download className="w-4 h-4" /></Button>
              ...
              <tr className="bg-white dark:bg-slate-800 border-b-2 border-slate-200 dark:border-slate-700">
                {hasSelection && (
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={selCount === paginated.length && paginated.length > 0}
      ...
      <div className="flex items-center justify-between text-sm">
        <span className="text-slate-500 nums-fa">
          {filtered.length > 0
        ...
        {totalPages > 1 && (
          <div className="flex items-center gap-1">
            ...
            <span className="px-3 nums-fa text-slate-600">{page} / {totalPages.toLocaleString("fa-IR")}</span>
