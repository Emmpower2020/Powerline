...
export function CreatePersonnelDialog({ open, onClose, onCreated }: { open: boolean; onClose: () => void; onCreated: () => void }) {
  const [submitting, setSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [form, setForm] = useState({ first_name: "", last_name: "", position: "", mobile: "", hire_date: "", contract_id: "" });

  const submit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.first_name) { setError("نام الزامی است"); return; }
    setSubmitting(true); setError(null);
    try { await apiClient.post(API_ENDPOINTS.personnel, { first_name: form.first_name, last_name: form.last_name, position: form.position || null, mobile: form.mobile || null, hire_date: form.hire_date || null, contract_id: form.contract_id ? Number(form.contract_id) : null }); setForm({ first_name: "", last_name: "", position: "", mobile: "", hire_date: "", contract_id: "" }); onCreated(); } catch (err) { setError(err instanceof Error ? err.message : "خطا"); } finally { setSubmitting(false); }
  };
